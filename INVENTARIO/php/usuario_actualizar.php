<?php
    ...
    $check_admin=$check_admin->query("SELECT usuario_nombre,usuario_clave FROM usuario WHERE usuario_nombre='$admin_usuario' ");
    if($check_admin->rowCount()==1){

    	$check_admin=$check_admin->fetch();

    	if($check_admin['usuario_nombre']!=$admin_usuario || !password_verify($admin_clave, $check_admin['usuario_clave'])){
    		echo '
	            <div class="notification is-danger is-light">
	                <strong>¡Ocurrio un error inesperado!</strong><br>
	                USUARIO o CLAVE_ de administrador incorrectos
	            </div>
	        ';
	        exit();
    	}
    ...
    /*== Verificando usuario ==*/
    if($nombre!=$datos['usuario_nombre']){
	    $check_usuario=conexion();
	    $check_usuario=$check_usuario->query("SELECT usuario_nombre FROM usuario WHERE usuario_nombre='$nombre'");
	    if($check_usuario->rowCount()>0){
	        echo '
	            <div class="notification is-danger is-light">
	                <strong>¡Ocurrio un error inesperado!</strong><br>
    ...
    $actualizar_usuario=$actualizar_usuario->prepare("UPDATE usuario SET usuario_nombre=:nombre,usuario_clave=:clave,usuario_email=:email WHERE usuario_id=:id");
